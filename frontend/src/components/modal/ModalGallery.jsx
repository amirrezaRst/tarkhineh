                ...
                <div className="flex gap-2 overflow-x-auto no-scrollbar">
                    {list.map((img, i) => (
                        <button
                            ...
                            className={`shrink-0 w-[62px] h-[52px] rounded-xl overflow-hidden border-2 duration-200
                                ${i === selected ? "border-primary opacity-100" : "border-transparent opacity-60 hover:opacity-90"}`}
                        >
                            <img src={`${IMAGE_URL}/${img}`} alt="" className="w-full h-full object-cover object-center" />
                        </button>
                    ))}
                </div>
            }
